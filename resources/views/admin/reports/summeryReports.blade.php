...
<title>{{websiteTitle('Summery Reports')}}</title>
@endsection @push('css')
...
<div class="content-header row">
    ...
    <div class="content-header-right col-md-6 col-12 mb-md-0 mb-2">
        <div class="btn-group float-md-right" role="group" aria-label="Button group with nested dropdown">
            <a class="btn btn-outline-primary" href="{{route('admin.reportsAll','summery')}}"> <i class="fa-solid fa-rotate"></i> </a>
        </div>
    </div>
</div>
            ...
            <div class="col-md-12">
                ...
                <div class="card">
                    <div class="card-header" style="border-bottom: 1px solid #e3ebf3;padding: 1rem;">
                        <h4 class="card-title" style="padding: 5px;">Summery Reports ({{$day}} Days)</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                           <div class="row">
                               ...
                               <div class="col-md-6">
                                   <form action="{{route('admin.reportsAll','summery')}}">
                                   <div class="input-group">
                                        <input type="date" name="startDate" value="{{Carbon\Carbon::parse($from)->format('Y-m-d')}}" class="form-control {{$errors->has('startDate')?'error':''}}" />
                                        <input type="date" value="{{Carbon\Carbon::parse($to)->format('Y-m-d')}}" name="endDate" class="form-control {{$errors->has('endDate')?'error':''}}" />
                           ...
                           <h3 style="font-weight: bold;font-family: sans-serif;">ORDERS REPORTS</h3>
                           <div class="row MyReports">
                               <div class="col-md-3">
                                   <h3>{{$reports['ordersTotal']}}</h3>
                                   <p>Total Orders</p>
                               </div>
                               <div class="col-md-3">
                                   <h3>{{$reports['ordersPending']}}</h3>
                                   <p>Pending Orders</p>
                               </div>
                               <div class="col-md-3">
                                    <h3>{{$reports['ordersConfirmed']}}</h3>
                                   <p>Confirmed Orders</p>
                               </div>
                               <div class="col-md-3">
                                    <h3>{{$reports['ordersShipped']}}</h3>
                                   <p>Shipped Orders</p>
                               </div>
                               <div class="col-md-3">
                                    <h3>{{$reports['ordersDelivered']}}</h3>
                                   <p>Completed Orders</p>
                               </div>
                               <div class="col-md-3">
                                    <h3>{{$reports['ordersCancelled']}}</h3>
                                   <p>Cancelled Orders</p>
                               </div>
                               <div class="col-md-3">
                                    <h3>{{$reports['ordersDue']}}</h3>
                                   <p>Due Orders</p>
                               </div>
                               <div class="col-md-3">
                                  <h3>{{$reports['ordersPaid']}}</h3>
                                  <p>Paid Orders</p>
                               </div>
                           </div>
                           <br>
                           <h3 style="font-weight: bold;font-family: sans-serif;">SALES REPORTS (<small>Orders</small>)</h3>
                           <div class="row MyReports">
                               <div class="col-md-3">
                                   <h3>{{priceFullFormat($reports['ordersSales'])}}</h3>
                                   <p>Total Sale</p>
                               </div>
                               <div class="col-md-3">
                                   <h3>{{priceFullFormat($reports['ordersSalesPaid'])}}</h3>
                                   <p>Total Paid</p>
                               </div>
                               <div class="col-md-3">
                                   <h3>{{priceFullFormat($reports['ordersSalesDue'])}}</h3>
                                   <p>Todal Due</p>
                               </div>
                               <div class="col-md-3">
                                   <h3>{{priceFullFormat($reports['ordersSales']/$day)}}</h3>
                                   <p>Average Sale</p>
                               </div>
                           </div>
                        </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        $(".dataex-visibility-print").DataTable({ dom: "Bfrtip", buttons: ["print"] }),
            $(".dataex-visibility-message").DataTable({ dom: "Bfrtip", buttons: [{ extend: "print", message: "This print was produced using the Print button for DataTables" }] }),
            $(".dataex-visibility-selector").DataTable({ dom: "Bfrtip", buttons: [{ extend: "print", exportOptions: { columns: ":visible" } }, "colvis"], columnDefs: [{ targets: -1, visible: !1 }] }),
            $(".dataex-visibility-rowselector").DataTable({
                dom: "Bfrtip",
                buttons: [
                    { extend: "print", text: "Print all" },
                    { extend: "print", text: "Print selected", exportOptions: { modifier: { selected: !0 } } },
                ],
                select: !0,
            }),
            $(".dataex-visibility-disable").DataTable({ dom: "Bfrtip", buttons: [{ extend: "print", autoPrint: !1 }] }),
                ...
                buttons: [
                    {
                        ...
                        customize: function (t) {
                            $(t.document.body).css("font-size", "10pt").prepend('<img src="http://datatables.net/media/images/logo-fade.png" style="position:absolute; top:0; left:0;" />'),
                                $(t.document.body).find("table").addClass("compact").css("font-size", "inherit");
                        },
                    },
                ],
            });
