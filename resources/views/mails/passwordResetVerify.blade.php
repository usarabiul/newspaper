...
<h2 style="margin:0;">{{$datas['user']->name}}</h2>
<p style="margin:0;">Welcome To {{general()->title}} !</p>
</center>

<div style="background:#f1f1f1;padding:10px;margin: 10px 0;">
  <p style="margin: 0;border-bottom: 1px solid #c6c6c6;padding: 5px 0;">RESET PASSWORD VERIFY CODE </p>
  <br>
  
  <p style="margin:0;"><strong>Name:</strong> {{$datas['user']->name}} </p>
  <p style="margin:0;"><strong>Mobile:</strong> {{$datas['user']->mobile}}</p>
  <p style="margin:0;"><strong>Email:</strong> {{$datas['user']->email}}</p>
  <center>
  <span>Verify Code: {{$datas['user']->verify_code}}</span><br>
  <a href="{{route('resetPassword',[$datas['user']->remember_token?:'notoken','code'=>$datas['user']->verify_code])}}">Click to Reset</a>
  </center>
  <br>
